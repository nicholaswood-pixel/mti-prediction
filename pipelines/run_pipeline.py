...
def main():  # pragma: no cover
    ...
    try:
        ...
        if os.getenv("PRINT_PIPELINE_DEFINITION", "0") == "1":
            print("###### Creating/updating a SageMaker Pipeline with the following definition:")
            parsed = json.loads(pipeline_definition)
            print(json.dumps(parsed, indent=2, sort_keys=True))
        else:
            print("###### Creating/updating a SageMaker Pipeline definition.")
        ...
        max_attempts = int(os.getenv("PIPELINE_WAIT_MAX_ATTEMPTS", "180"))
        delay_seconds = int(os.getenv("PIPELINE_WAIT_DELAY_SECONDS", "60"))
        execution.wait(max_attempts=max_attempts, delay=delay_seconds)
